    ...
    loop {
        ...
        match lora.rx(&rx_packet_params, &mut buf).await {
            Ok((received_len, _rx_pkt_status)) => {
                ...
                match packet.kind {
                    PacketKind::Open(ZoneId(id)) => {
                        let prev = ZONE_STATUS_SIGNAL.wait().await;
                        match id {
                            1 => ZONE_STATUS_SIGNAL.signal([ZoneStatus::Open, prev[1]]),
                            2 => ZONE_STATUS_SIGNAL.signal([prev[0], ZoneStatus::Open]),
                            other => info!("Invalid zone id: {}", other),
                        }
                    }
                    PacketKind::Close(ZoneId(id)) => todo!(),
                }
            }
